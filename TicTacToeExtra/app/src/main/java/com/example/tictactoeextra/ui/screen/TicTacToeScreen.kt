package com.example.tictactoeextra.ui.screen
...
import com.example.tictactoeextra.R
...
fun TicTacToeScreen(
    ...
    Column(
        ...
        Text("X:  $xWins      O:  $oWins", fontSize = 20.sp)
        TicTacToeBoard(
            viewModel.board,
            onBoardCellClicked = {
                viewModel.onCellClicked(it)
                // 2
                val result = checkForWinner(viewModel.board)
                if (result != null) {
                    winner = result
                    if (result == Player.X) {
                        xWins++
        ...
        // 4
        if (winner != null) {
            Text(text = "Player ${winner!!.name} has won!", fontSize = 24.sp)
        }
    }
}
...
fun checkForWinner(board: Array<Array<Player?>>): Player? {
    // Check rows
    for (row in board) {
